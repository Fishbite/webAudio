/* // ****** LICENSE NOTICE ****** \\
      This code has been created from Rex van der Spuy's book:
      'Advanced Game Design With HTML5 and JavaScript'

      Modifications have been made to link the `Sound` Class' audio graph
      to a main recording chain in recordingChain.js.js
...
// Commented out, because we are using the global audio context
...
class Sound {
  constructor(source, loadHandler) {
    ...
    this.actx = actx;
    // this.mainVol = mainVol; // mainVol connects us to the global recording chain
  ...
  play() {
    ...
    // If we have a global recording chain, connect to it
    if (mainVol) {
      this.panNode.connect(mainVol);
    } else {
...
export function soundEffect(
  ...
  if (mainVol) pan.connect(mainVol);
  ...
  function addDissonance() {
    ...
    if (mainVol) d1Volume.connect(mainVol);
    d2.connect(d2Volume);
    d2Volume.connect(actx.destination);
    if (mainVol) d2Volume.connect(mainVol);
